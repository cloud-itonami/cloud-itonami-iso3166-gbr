    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(VAT/統一資格/Find a Tender登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- uk-entity-missing-violations
...
(defn- vat-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-vat? true`, INDEPENDENTLY check
  `:vat-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-vat? e))
                 (not (true? (:vat-verified? e))))
        [{:rule :vat-unverified
          :detail (str subject " はVAT確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (uk-entity-missing-violations request st)
