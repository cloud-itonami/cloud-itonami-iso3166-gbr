(ns marketentry.marketentryllm
  ...
  the GBR public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-uk-entity?=" (:has-uk-entity? e)
                        " vat-verified?=" (:vat-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-uk-entity? e))
                              (:has-uk-entity? e))
                          (or (not (:requires-vat? e))
                              (:vat-verified? e)))
                   0.9 0.3)}))
