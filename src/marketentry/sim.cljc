...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (GBR, clean) ==")
    (println (exec-op actor "t1" {:op :engagement/intake :subject "eng-1"
                                  :patch {:id "eng-1" :operator "Kita Systems GBR"}} operator))
    ...
    (println "== jurisdiction/assess eng-4 (sets up uk-entity-missing) ==")
    ...
    (println "== filing/submit eng-4 (uk-entity-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up vat-unverified) ==")
    ...
    (println "== filing/submit eng-5 (vat-unverified -> HARD hold) ==")
