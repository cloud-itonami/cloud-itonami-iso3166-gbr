(ns marketentry.store
  "SSoT for the GBR market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems GBR" :portal "Find a Tender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-uk-entity? true :has-uk-entity? true
             :requires-vat? true :vat-verified? true
             :drafted? false :submitted? false
             :jurisdiction "GBR" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "Find a Tender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-uk-entity? true :has-uk-entity? true
             :requires-vat? true :vat-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "Find a Tender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "GBR" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "Find a Tender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-uk-entity? true :has-uk-entity? false
             :requires-vat? true :vat-verified? true
             :drafted? false :submitted? false
             :jurisdiction "GBR" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "Find a Tender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-uk-entity? true :has-uk-entity? true
             :requires-vat? true :vat-verified? false
             :drafted? false :submitted? false
             :jurisdiction "GBR" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "Find a Tender"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-uk-entity? has-uk-entity?
                               requires-vat? vat-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-uk-entity?)  (assoc :engagement/requires-uk-entity? requires-uk-entity?)
    (some? has-uk-entity?)       (assoc :engagement/has-uk-entity? has-uk-entity?)
    (some? requires-vat?)    (assoc :engagement/requires-vat? requires-vat?)
    (some? vat-verified?)    (assoc :engagement/vat-verified? vat-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-uk-entity? :engagement/has-uk-entity?
   :engagement/requires-vat? :engagement/vat-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-uk-entity? (boolean (:engagement/requires-uk-entity? m))
     :has-uk-entity? (boolean (:engagement/has-uk-entity? m))
     :requires-vat? (boolean (:engagement/requires-vat? m))
     :vat-verified? (boolean (:engagement/vat-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-gbr"})
