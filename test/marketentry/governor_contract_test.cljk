...
(deftest clean-intake-auto-commits
  (let [[db actor] (fresh)
    ...
    (is (= "Kita Systems GBR" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest uk-entity-missing-is-held-and-unoverridable
  (testing "missing Japan-resident authorized representative -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:uk-entity-missing} (-> (store/ledger db) last :basis)))
